...
const BindModelForm = ({ visible, onHideModal, model }) => {
    ...
    const dataSourceColumns = [
        { dataIndex: 'dataSourceId', key: 'dataSourceId', title: '数据源ID' },
        { dataIndex: 'dataSourceName', key: 'dataSourceName', title: '数据表' },
        {
            key: 'action',
            title: '操作',
            render: (_, row) => (
                <Button danger onClick={() => handleDeleteDataSource(row.key)}>
                    删除
                </Button>
            )
        }
    ...
    const handleAddDataSource = () => {
        bindForm
            ...
            .then((data) => {
                ...
                const filteredFieldMap = Object.fromEntries(
                    Object.entries(fieldMap).filter(([_, value]) => !!value)
                );
    ...
    const fetchFields = (dataSource) => {
        ...
        let { dataSourceId, description, fieldMap, key, tag, type, ...params } =
            dataSource;
        params.sourceType = type;
        getMetaFields(params)
            .then((data) => {
                // 直接按照顺序匹配
                const recommendedFields = data.map((item, index) => ({
                    name: item.name,
                    type: item.type,
                    recommendedField: model.fields[index]?.name || ''
                }));
                setDataSourceFields(recommendedFields);
                loading();
            })
            .catch((error) => {
                loading();
    ...
    const filterDatasource = (input, option) => {
        const dataSource = dataSources.find(
            (item) => item.dataSourceId === option.value
        );
        return formatDataSourceTable(dataSource)
            .toLowerCase()
    ...
    const validateField = (rule, val) => {
        ...
        const sourceType = dataSourceFields.find(
        ...
        const mappedFields = dataSourceFields
            .map((item) => {
                  ...
                  <span style={{ marginRight: '16px' }}>
                    ...
                    {dataSourceFields?.map((item) => (
                        <Form.Item
                            ...
                            label={
                                <>
                                    <span>{formatMongoField(item.name)}</span> {/* 原字段名称 */}
                                    {item.recommendedField && (
                                        <Tag color="blue" style={{ marginLeft: 12 }}>
                                            推荐: {item.recommendedField} {/* 显示推荐的字段 */}
                                        </Tag>
                                    )}
                            ...
                            name={item.name}
                            initialValue={item.recommendedField}
