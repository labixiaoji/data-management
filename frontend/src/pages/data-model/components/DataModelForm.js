...
import {addDataModel, querySimilarTags} from '../../../apis/data-model'
import {formatVariableType, formatRuleType, formatRuleDetail} from '../methods'

const {Step} = Steps
const {Option} = Select

const DataModelForm = ({visible, onHideModal, onAddModel}) => {
        ...
        <Modal title='添加数据模型' open={visible} onOk={handleOk} onCancel={resetModal} width={800}>
            ...
            {currentStep === 1 && (<>
                ...
                <Form layout='vertical' form={fieldForm} style={{marginTop: '20px'}}>
                    ...
                    <Form.Item name='sensitive' label='是否为key值' valuePropName='checked' initialValue={false}>
                        <Switch checkedChildren='key值' unCheckedChildren='非key值'/>
                    </Form.Item>
